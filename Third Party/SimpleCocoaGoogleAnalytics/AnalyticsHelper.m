...
- (void)handleApplicationWillTerminate {
    ...
    [self recordCachedEventWithCategory:@"Application" action:@"Terminate" label:dateString value:durationInSeconds];
...
- (NSData*)createEventPayload:(AnalyticsEvent*)analyticsEvent
    ...
    NSMutableString *payloadString = [NSMutableString stringWithFormat:@"v=1&tid=%@&cid=%@&an=%@&t=event&ec=%@&ea=%@",
    ...
    if (analyticsEvent.label) {
        [payloadString appendFormat:@"&el=%@", analyticsEvent.label];
    }
    if (analyticsEvent.value) {
        [payloadString appendFormat:@"&ev=%@", analyticsEvent.value];
    }
    
    NSString *escapedPayloadString = [payloadString stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    
    return [escapedPayloadString dataUsingEncoding:NSUTF8StringEncoding];
}
